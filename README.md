...
リポジトリに含まれるサンプルで対応機能を確認できます:

```bash
uv run marpx examples/demo/demo.md
```
...
- インライン数式（`$...$`）-- 段落内に編集可能な OMML 数式として統合
- ブロック数式（`$$...$$`）-- 編集可能な OMML 数式シェイプとして配置
- 単色背景
...
- **数式**（MathJax） -- LaTeX ソースが利用可能な場合は OMML（Office Math Markup Language）による編集可能な数式として挿入。インライン数式（`$x^2$`）はテキスト段落内に OMML として直接埋め込まれ、前後のテキストと同一テキストボックス内で自然にリフロー・編集が可能。ブロック数式（`$$...$$`）は独立したシェイプとして配置。LaTeX ソースが取得できない場合は 2x DPI フォールバック画像として出力
...
`examples/` ディレクトリに Marp プレゼンテーションのサンプルが含まれています:

| ディレクトリ | 内容 |
|-------------|------|
| `examples/demo/` | 対応機能を網羅したデモスライド（`demo.md` + 画像） |
| `examples/css_showcase/` | CSS テーマのショーケース（`quantum.md`、`deep_sea.md`、`space.md`） |
...
# CSS ショーケースを変換
uv run marpx examples/css_showcase/quantum.md
```
...
| `extraction/math_preprocessor.py` | 数式の LaTeX ソースを data-latex 属性で保持する Markdown 前処理 |
...
| `utils/math.py` | LaTeX → MathML → OMML 変換（MML2OMML.XSL 使用） |
...
| `pptx_builder/math.py` | 編集可能な数式の OMML 挿入 |
